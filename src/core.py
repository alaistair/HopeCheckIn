...
def display_family(family, df_people):
    ...
    for person in family['First Name']:
        ...
        if centre.checkbox('Check in', value=checked_in_already, key=person+'in'):
            df_people.at[member_row, 'Checked In'] = datetime.now(tz=None)
            #if right.button('Print label', key=person+'print'):
            #    print_label(member_id)
            #    st.write("printing for " + person)
        else:
...
def print_label(member_id):
    ...
    PATH = str(os.path.realpath('..')) + '/client-hope/'
    ...
    try:
        ...
        print('Print error for ' + FILE_PATH)
